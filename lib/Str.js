...
class Str {
    ...
    signTransaction(path, transaction) {
        ...
        return (0, utils_1.foreach)(apdus, (data, i) => this.transport
            .send(CLA, keepAlive ? INS_KEEP_ALIVE : INS_SIGN_TX, i === 0 ? P1_FIRST_APDU : P1_MORE_APDU, i === apdus.length - 1 ? P2_LAST_APDU : P2_MORE_APDU, data, [
            SW_OK,
            SW_CANCEL,
            SW_UNKNOWN_OP,
            SW_MULTI_OP,
     ...
     */
    signSorobanAuthoration(path, hashIdPreimage) {
        (0, utils_1.checkStellarBip32Path)(path);
        const apdus = [];
        let response;
        const pathElts = (0, utils_1.splitPath)(path);
        ...
        let keepAlive = false;
        return (0, utils_1.foreach)(apdus, (data, i) => this.transport
            .send(CLA, keepAlive ? INS_KEEP_ALIVE : INS_SIGN_SOROBAN_AUTHORATION, i === 0 ? P1_FIRST_APDU : P1_MORE_APDU, i === apdus.length - 1 ? P2_LAST_APDU : P2_MORE_APDU, data, [SW_OK, SW_CANCEL, SW_KEEP_ALIVE, SW_CUSTOM_CONTRACT_MODE_NOT_ENABLED])
            .then(apduResponse => {
            const status = Buffer.from(apduResponse.slice(apduResponse.length - 2)).readUInt16BE(0);
            if (status === SW_KEEP_ALIVE) {
                keepAlive = true;
